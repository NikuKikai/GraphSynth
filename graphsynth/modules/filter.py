...
class _BiquadBase(Module):
    def __init__(self, freq=1000.0, Q=0.707, sr=44100):
        super().__init__()
    ...
    def _proc(self, signal: NoteSignal, inp: np.ndarray = 0.0, freq: np.ndarray = 1000.0, Q=0.707, gain=0.0) -> dict:
        # Load state
        ...
        if state_key not in signal._states:
            ...
            signal._states[state_key] = (0.0, 0.0)  # z1, z2
        z1, z2 = signal._states[state_key]

        # Process
        out = np.zeros_like(inp)
        for i, x_args in enumerate(zip_longest(inp, freq, Q, gain)):
            x, *_args = x_args
        ...
        # Save state
        signal._states[state_key] = (z1, z2)
        return {'out': out}
...
class PeakingEQ(_BiquadBase):
    def __init__(self, freq=1000, Q=0.707, gain=0.0, sr=44100):
        super().__init__(freq, Q, sr)
...
# class EQ:
#     def __init__(self, bands: int = 10, sr: int = 44100, Q: float = 1.0):
#         self.sr = sr
#         self.bands = bands
#         self.Q = Q
...
#             bands
#         )
#         print("EQ frequencies:", self.freqs)

#         self.gains = [0.0] * bands
#         self.sections = []

#         for f in self.freqs:
#             coeff = _biquad_coeff("peaking", f, Q, 0.0, sr)
#             self.sections.append(Biquad(*coeff))

#     def set_gain(self, band_index: int, gain: float):
#         self.gains[band_index] = gain
#         f = self.freqs[band_index]

#         coeff = _biquad_coeff("peaking", f, self.Q, gain, self.sr)
#         self.sections[band_index] = Biquad(*coeff)
